...
module.exports = (robot) ->
  ...
  robot.on "github-repo-event", (issues) -> 
         ...
         if issues.eventType == 'issue_comment'      
          robot.send { room: issues.query.room, event: issues.eventType }, issues.payload.issue.url + '\n' + "new comment added:  " + issues.payload.comment.body + '\n' + "commented by:  " + issues.payload.issue.user.login
          LATEST_ISSUE = issues.payload.issue.url + '\n' + "new comment added:  " + issues.payload.comment.body + '\n' + "commented by:  " + issues.payload.issue.user.login +" \n " + "Created_at: " + issues.payload.issues.created_at

   robot.on "github-repo-event", (issues) -> 
        console.log("issues for push", arguments);   
        if issues.eventType == 'push'      
         robot.send { room: issues.query.room, event: issues.eventType }, issues.payload.pusher.name + '\n' + 'file name: ' + issues.payload.repository.name + ' \n' + 'Created_at: '+ issues.payload.created + " new push added"
         LATEST_ISSUE = issues.payload.pusher.name + '\n' + 'file name: ' + issues.payload.repository.name + '\n'  + 'Created_at: '+ issues.payload.created + " new push added"

    robot.on "github-repo-event", (issues) -> 
        console.log("issues for issues event", arguments);   
        if issues.eventType == 'issues'
         if issues.payload.action == 'opened'      
          robot.send { room: issues.query.room, event: issues.eventType }, 'The link:  ' + issues.payload.issue.url + '\n' + "Title:  " + issues.payload.issue.title + '\n' + "Issued By: " + issues.payload.issue.user.login + '\n' + "Created At: " + issues.payload.issue.created_at
          LATEST_ISSUE = 'The link:  ' + issues.payload.issue.url + '\n' + "Title:  " + issues.payload.issue.title + '\n' + "Issued By: " + issues.payload.issue.user.login + '\n' + "Created At: " + issues.payload.issue.created_at
    ...
    robot.on "github-repo-event", (issues) -> 
        ...
        if issues.eventType == 'pull_request'
         if issues.payload.action == 'opened'      
          ...
          robot.send { room: issues.query.room, event: issues.eventType }, 'Merged_at:  ' + issues.payload.pull_request.merged_at + "\n" + "Title:  " + issues.payload.pull_request.title + '\n' +  "Issued By: " + issues.payload.pull_request.user.login + "\n" + "For Commit URL: " + issues.payload.pull_request.commits_url
    ...
    robot.on "github-repo-event", (issues) -> 
        ...
        if issues.eventType == 'pull_request_review_comment'      
         robot.send { room: issues.query.room, event: issues.eventType }, 'The link:  ' + issues.payload.comment.url + "\n" + "Path:  " + issues.payload.comment.path + '\n' + "Issued By: " + issues.payload.comment.user.login + + "Created_at: " + issues.payload.comment.created_at  + "\n"  + "Body: " + issues.payload.comment.body
         LATEST_ISSUE = 'The link:  ' + issues.payload.comment.url + "\n" + "Path:  " + issues.payload.comment.path + '\n' + "Issued By: " + issues.payload.comment.user.login + + "Created_at: " + issues.payload.comment.created_at  + "\n"  + "Body: " + issues.payload.comment.body
